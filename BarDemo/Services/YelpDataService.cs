...
namespace BarDemo.Services
{
    public class YelpDataService : BaseHttpService
    {
        ...
        public async Task<BarSearch> BusinessSearch()
        {
            // https://api.yelp.com/v3/businesses/search?term=delis&latitude=29.425688&longitude=-98.493720&limit=5
            var url = new Uri(_baseUri, "businesses/search?location=austin&limit=2");
            BarSearch response = await SendRequestAsync<BarSearch>(url, HttpMethod.Get, _headers);
